...
router.get('/get_weight/:type', async (request, response) => {
...
router.get('/get_trash_data/:type', async (request, response) => {
  ...
  let query = `SELECT id, weight, time_added, trash_type_id FROM trash WHERE trash_round = (${round}) AND trash_accommodation_id = (${accom_id}) AND trash_type_id = (${trash_type}) ORDER BY time_added DESC LIMIT 3 `;
